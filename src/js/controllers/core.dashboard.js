...
// Form controller
app.controller("CoreDashboardCtrl", ["$scope", "$rootScope", "locker", "Analytics", "$filter", "Store", "$state", "$mdToast", "$animate", "Social","Spark", "$timeout","Product",
    function(                           $scope,   $rootScope,   locker,   Analytics,   $filter, Store,  $state,   $mdToast,   $animate, Social, Spark, $timeout,Product) {
    ...
    Product.ProductAllSearchedData(startRefreshName,endRefreshName,productName,merchantToken).then(function() {
        ...
        $scope.productFilter = function(){
            ...
            let productName = $scope.product.selected.productName;
            let productData = {
                Product : productName,
            ...
            $scope.updatefilterresponse = function(statusCode,statusMessage,responseData){
                if(statusCode == 0){
                    ...
                    let totalParticipant  = responseData == undefined ? 0: responseData[0].totalParticipants;
                    let totalRedemption   = responseData[0].totalRedemptionRate == undefined ? 0:responseData[0].totalRedemptionRate;
                    let totalSales        = responseData[0].totalSalesAmount == undefined ? 0:responseData[0].totalSalesAmount;
...
$scope.incomeFilter = function(){
    $scope.toggleWizardLoader.showLoader = true;
    let productName =  $scope.product.selected.productName;
    let minIncome  = $scope.rangeIncome.minIncome;
    let maxIncome  = $scope.rangeIncome.maxIncome;
    let incomeData = {
        Product       : productName,
        MinIncome     : minIncome,
        MaxIncome     : maxIncome,
    ...
    Spark.productGet(incomeData).then(response =>{
        $scope.toggleWizardLoader.showLoader = false;
    let statusCode = response.FlikResponse.Status.StatusCode;
    let statusMessage = response.FlikResponse.StatusMessage;
    let responseData = response.FlikResponse.Data.Spark.Filter;
    if(statusCode == 0){
        let totalParticipant = responseData == undefined ? 0: responseData[0].totalParticipants;
        let totalRedemption  = responseData == undefined ? 0:responseData[0].totalRedemptionRate;
        let totalSales       = responseData == undefined ? 0:responseData[0].totalSalesAmount;
        $scope.d3TotalParticipant = totalParticipant;
        $scope.d3TotalRedemption  = totalRedemption;
        $scope.d3TotalSales       = totalSales;
        $scope.csvData = [{"productName":productName,"totalParticipant":totalParticipant,"totalRedemption":totalRedemption,"totalSales":totalSales}];
    ...
    $mdToast.show(
        $mdToast.simple()
            .content("error in getting Income filter data please try again")
            .position("top right")
            .hideDelay(2700)
    );
...
$scope.ageFilter = function(){
    $scope.toggleWizardLoader.showLoader = true;
    let productName =  $scope.product.selected.productName;
    let minAge  = $scope.rangeAge.minAge;
    let maxAge  = $scope.rangeAge.maxAge;
    let incomeData = {
        Product       : productName,
        MinAge        : minAge,
        MaxAge        : maxAge,
    ...
    $mdToast.show(
        $mdToast.simple()
            .content("error in getting Age filter data please try again")
            .position("top right")
            .hideDelay(2700)
    );
...
$scope.genderFilter = function(){
    $scope.toggleWizardLoader.showLoader =true;
    let productName =  $scope.product.selected.productName;
    let gender = $scope.group.selected.groupBy;
    let genderData = {
        Product       : productName,
        Gender        : gender,
        MerchantToken : merchantToken
    };
    Spark.productGet(genderData).then(response =>{
        $scope.toggleWizardLoader.showLoader = false;
    let statusCode = response.FlikResponse.Status.StatusCode;
    let statusMessage = response.FlikResponse.Status.StatusMessage;
    let responseData = response.FlikResponse.Data.Spark.Filter;
    if(statusCode == 0){
        let femaleTotalParticipant = responseData == undefined ? 0: responseData.Female[0].totalParticipants;
        let maleTotalParticipant = responseData == undefined ? 0: responseData.Male[0].totalParticipants;
        let femaleTotalRedemption = responseData == undefined ? 0: responseData.Female[0].totalRedemptionRate;
        let maleTotalRedemption = responseData == undefined ? 0: responseData.Male[0].totalRedemptionRate;
        let femaleTotalSales = responseData == undefined ? 0: responseData.Female[0].totalSalesAmount;
        let maleTotalSales = responseData == undefined ? 0: responseData.Male[0].totalSalesAmount;
...
//             $scope.genderLayout();
    ...
    $mdToast.show(
        $mdToast.simple()
            .content("error in getting Gender filter data please try again")
            .position("top right")
            .hideDelay(2700)
    );
...
$scope.bubbleGraph = function(gData){
    ...
    var canvas = d3.select(".d3Graph").append("svg")
        .attr("id", "bubble")
        .attr("width", width)
        .attr("height", height);
    ...
    var circles = canvas.selectAll("circle")
        .data(gData)
        .enter()
        .append("circle").on('mouseover', tip.show)
    ...
    circleAttributes.transition().duration(2000).attr("r", function(d) {return d.radius});
    ...
    force.on("tick",function(){
        circles.attr("cx", function(d) {return d.x; })
            .attr("cy", function(d) { return d.y; }).call(force.drag);
    });
    ...
    $scope.genderLayout  = function(){
        force.gravity(0.10).friction(0.6).charge(function(d){return -Math.pow(d.radius, 2.0) / 8; }).on("tick",function(e){
            circles.each($scope.genderMovePosition(e.alpha)).attr("cx",function(d){return d.x;}).attr("cy", function(d){return d.y}).call(force.drag);
        });
        force.start();
    ...
    $scope.genderTextMove = function(){
        ...
        var yPostion = 50;
        var textField =canvas.selectAll("text").data(data)
            .enter().append("text").attr("class", "gender").attr("x", function(d){return datas[d];})
            .attr("y",50).attr("text-anchor", "middle").attr("font-size", "20px").text(function(d){
                return d;});
    };

    if($scope.group.selected.groupBy == 'Gender'){
        $timeout(function(){$scope.genderLayout();}, 3000) ;
    }
};
...
$scope.locationTargeting = () =>{
    $scope.heatMapList = [];
    $scope.locationCsv = [];
    let productName = $scope.product.locationselected.productName;
    let locationData = {
        Product      : productName,
        MerchantToken: merchantToken,
    };
    $scope.updateresponse = function(statusCode,statusMessage,responseData){
        ...
        if(statusCode == 0){
            if(responseData == 0){
                $mdToast.show(
                    $mdToast.simple()
                        .content("Product "+productName+" have no location Transaction")
                        .position("top right")
                        .hideDelay(2700)
                );
            }
            angular.forEach(responseData, function(StateData, index){
                let productName = $scope.product.locationselected.productName;
                let totalParticipant = StateData.totalParticipants;
                let totalRedemption = StateData.totalRedemptionRate == undefined ? 0: StateData.totalRedemptionRate;
                let totalSales = StateData.totalSalesAmount == undefined ? 0: StateData.totalSalesAmount;
                ...
                let dataToShow = StateData.totalParticipants;
                $scope.tab = "Participant";
                let lat = StateData.latitude;
                let lng = StateData.longitude;
                //let lat = "37.33233141"
                //let lng = "-122.0312186"
        ...
        $mdToast.show(
            $mdToast.simple()
                .content("error in getting Location Data please try again")
                .position("top right")
                .hideDelay(2700)
        );
    ...
    if ($scope.heatmapcampaigntype.value == 'Social'){
        Social.productSocialLocationGet(locationData).then(response =>{
        let statusCode    = response.FlikResponse.Status.StatusCode;
        let statusMessage = response.FlikResponse.Status.StatusMessage;
        let responseData  = response.FlikResponse.Data.Social.Filter.length == 0 ? 0 : response.FlikResponse.Data.Social.Filter;
        /**
    ...
    Spark.productLocationGet(locationData).then(response =>{
    let statusCode    = response.FlikResponse.Status.StatusCode;
    let statusMessage = response.FlikResponse.Status.StatusMessage;
    let responseData  = response.FlikResponse.Data.Spark.Filter.length == 0 ? 0 : response.FlikResponse.Data.Spark.Filter;
    /**
     * to access this response data from tab change function.
     *
     */
...
$scope.selectedTab =(tabSelected) =>{
    ...
    angular.forEach($scope.allStateData, function(value, index){
        ...
        let dataToShow = value[tabSelected];
...
$scope.map = function(){
        ...
        angular.forEach($scope.heatMapList, function(stateValue, index){
            if(parseInt(selectedLatLng.lat) == parseInt(stateValue.lat) && parseInt(selectedLatLng.lng) == parseInt(stateValue.lng)){
                ...
                var popup = L.popup()
                    ...
                    .setContent("<strong>"+$scope.tab+":</strong> <span style='color:red'>" + stateValue.count + "</span>")
                    .openOn(map);
            }
    ...
    Store.storeGetAll(locationData).then(response => {
    let statusCode = response.FlikResponse.Status.StatusCode;
    let statusMessage = response.FlikResponse.Status.StatusMessage;
    let responseData = response.FlikResponse.Data.MerchantStoreAll.StoreList.length == 0 ? 0 : response.FlikResponse.Data.MerchantStoreAll.StoreList;
    $scope.allStoreData = responseData;
    if(statusCode == 0) {
        if (responseData == 0) {
            $mdToast.show(
                $mdToast.simple()
                    .content("Currently, no stores found.")
                    .position("top right")
                    .hideDelay(2700)
            );
        }
...
let colors = hCharts.getOptions().colors;
