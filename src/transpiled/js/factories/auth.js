...
angular.module('app').factory('Auth', ['$state', '$q', 'locker', 'Restangular', function ($state, $q, locker, Restangular) {
  ...
  return {
    isLoggedIn: function isLoggedIn() {
      ...
      rAuthCheck.post(token).then(function (response) {
    ...
    login: function login(merchantCreds) {
      return rAuthMerchant.post(merchantCreds);
    },
    getMe: function getMe(merchantToken) {
      var token = {
        MerchantToken: merchantToken
      };
      return rMerchantMe.post(token);
    },
    logout: function logout(merchantToken) {
      var token = {
        tokenKey: merchantToken
      };
      return rAuthLogout.post(token);
    },
    signup: function signup(merchantInfo) {
     ...
     *
     */
    newPassword: function newPassword(emailId) {
