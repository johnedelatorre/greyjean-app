...
app.controller("CoreOfferCreateCtrl", ["$scope", "$rootScope", "$state", "$stateParams", "$filter", "$http", "Offer", "locker", "$mdToast", "$mdDialog", "$timeout", "$animate", "Product", function ($scope, $rootScope, $state, $stateParams, $filter, $http, Offer, locker, $mdToast, $mdDialog, $timeout, $animate, Product) {
  ...
  locker.namespace("offerCreate").bind($scope, "offerName");
  $scope.offerName = "";
  locker.namespace("offerCreate").bind($scope, "offerAmount");
  $scope.offerAmount = 40;
  locker.namespace("offerCreate").bind($scope, "offerZipcode");
  $scope.offerZipcode = 91604;
  locker.namespace("offerCreate").bind($scope, "offerStartDate");
  $scope.offerStartDate = moment(todayNow).format("DD-MMMM-YYYY");
  locker.namespace("offerCreate").bind($scope, "offerEndDate");
  $scope.offerEndDate = moment($scope.offerStartDate).add(1, "months").format("DD-MMMM-YYYY");
  $scope.offerType = "";
  locker.namespace("offerCreate").bind($scope, "offerDescription");
  ...
  locker.namespace("offerCreate").bind($scope, "offer");
  $scope.offer = {};
  locker.namespace("offerCreate").bind($scope, "offerType");
  $scope.offerType = {};
  ...
  var tFilter = $scope.transactionFilter;
  ...
  $scope.createConBuyList = function () {
    var filterId = $scope.conBuysLists.length + 1;
    if ($scope.conBuysLists.length < 5) {
      ...
      $mdToast.show($mdToast.simple().content("maximum 5 transaction filters allowed").position("top right").hideDelay(2700));
    }
    console.info("this are ze con lists" + JSON.stringify($scope.conBuysLists, null, 4));
  };

  $scope.trashConBuyList = function (index) {
  ...
  locker.namespace("offerCreate").bind($scope, "productCategories.selected");
  $scope.productCategories = {};
  $scope.productCategories.selected = [];

  locker.namespace("offerCreate").bind($scope, "productBrands.selected");
  $scope.productBrands = {};
  $scope.productBrands.selected = [];

  locker.namespace("offerCreate").bind($scope, "productTypes.selected");
  $scope.productTypes = {};
  $scope.productTypes.selected = [];

  var oneDayAgo = moment().subtract(1, "days").startOf("day").format("YYYY-MM-DD hh:mm:ss");
  var oneWeekAgo = moment().subtract(1, "weeks").startOf("day").format("YYYY-MM-DD hh:mm:ss");
  var oneMonthAgo = moment().subtract(1, "months").startOf("day").format("YYYY-MM-DD hh:mm:ss");
  var oneYearAgo = moment().subtract(1, "years").startOf("day").format("YYYY-MM-DD hh:mm:ss");
  ...
  locker.namespace("offerCreate").bind($scope, "timeFrames.selected");
  $scope.timeFrames = {};
  ...
  **/
  $scope.val = 5;
  var updateModel = function updateModel(val) {
  ...
  *
  **/
  $scope.goBackToFingerprint = function (ev) {
    var confirm = $mdDialog.confirm().title("Do you want to edit your last fingerprint?").content("You can create a new fingerprint if you like.").ariaLabel("Lucky day").ok("Edit my last fingerprint").cancel("Create a new fingerprint").targetEvent(ev);
    $mdDialog.show(confirm).then(function () {
      $scope.editLastFingerprint = true;
      locker.driver("session").namespace("offerCreate").put("editLastFingerprint", $scope.editLastFingerprint);
   ...
   */
  $scope.saveOffer = function (goNext) {
    var oName = $scope.offerName;
    var hasMultipleTransactions = !isNaN($scope.conBuysLists[0].filter.offerAmount) || Object.keys($scope.conBuysLists[0].filter.offerAmount).length > 0 || $scope.conBuysLists[0].filter.logicTransaction == "BUYS" ? true : false;
    var hasOfferTransaction = !isNaN($scope.transactionMainFilter.filter.offerAmount) || Object.keys($scope.transactionMainFilter.filter.offerAmount).length > 0 || hasMultipleTransactions ? true : false;
    ...
    if (!oName) {
      ...
      $mdToast.show($mdToast.simple().content("Please enter a Offer name").position("top right").hideDelay(2700));
      return;
    }
    if (!hasOfferTransaction) {
      $scope.toggleWizardLoader.showLoader = false;
      $mdToast.show($mdToast.simple().content("Please Select Offer Amount").position("top right").hideDelay(2700));
      return;
    }
    ...
    var shouldSaveLocation = Object.keys($scope.offerLocationAddresses.selected).length > 0 ? true : false;
    ...
    var oID = locker.driver("local").namespace("offers").count();
    var offerAmount = isNaN($scope.transactionMainFilter.filter.offerAmount) ? $scope.transactionMainFilter.filter.offerAmount.amount === undefined ? $scope.getsAmount : $scope.transactionMainFilter.filter.offerAmount.amount : $scope.transactionMainFilter.filter.offerAmount;
    var brandId = Object.keys($scope.transactionMainFilter.filter.productBrand.selected).length > 0 ? $scope.transactionMainFilter.filter.productBrand.selected.brandId : 3;
    var brandName = Object.keys($scope.transactionMainFilter.filter.productBrand.selected).length > 0 ? $scope.transactionMainFilter.filter.productBrand.selected.brandName : "ALL";
    ...
    Offer.create(offer).then(function (response) {
      ...
      if (statusCode == 0) {
        ...
        var _oID = response.FlikResponse.Data.MerchantOffer.MerchantOfferId;
        console.info(JSON.stringify(_oID, null, 4));
        locker.driver("session").namespace("offerCreate").put("activeOfferId", _oID);
        ...
        if (shouldSaveLocation && !hasMultipleTransactions) {
          if (goNext) {
            var goToCampaign = true;
            $scope.saveOfferLocation(hasMultipleTransactions, _oID, goToCampaign);
          } else {
            $scope.saveOfferLocation(hasMultipleTransactions, _oID);
          }
          ...
          if (goNext) {
            var goToCampaign = true;
            $scope.saveOfferTransactions(_oID, goToCampaign);
          } else {
            $scope.saveOfferTransactions(_oID);
          }
        } else {
          $scope.toggleWizardLoader.showLoader = false;
          $mdToast.show($mdToast.simple().content("offer " + $scope.offerName + " Saved").position("top right").hideDelay(2700));
      ...
      $scope.toggleWizardLoader.showLoader = false;
      $mdToast.show($mdToast.simple().content("error saving Offer " + oName + " please try again").position("top right").hideDelay(2700));
   ...
   */
  $scope.saveOfferLocation = function (shouldSaveTransaction, offerId, goNext) {
    /**
    ...
    var oID = offerId;
    var timeNow = moment().format("YYYY-MM-DD hh:mm:ss");
    var offerLocationLat = $scope.offerLocationAddresses.selected.geometry.location.lat.toString();
    var offerLocationLng = $scope.offerLocationAddresses.selected.geometry.location.lng.toString();
    var offerLocationAddress = $scope.offerLocationAddresses.selected.formatted_address;
    ...
    Offer.createLocation(offerLocation).then(function (response) {
      var statusCode = response.FlikResponse.Status.StatusCode;
      var statusMessage = response.FlikResponse.Status.StatusMessage;
      if (statusCode == 0) {
        var _timeNow2 = moment().format("YYYY-MM-DD hh:mm:ss");
        var oLocationID = response.FlikResponse.Data.MerchantOfferLocation.OfferLocationId;
        ...
        if (shouldSaveTransaction) {
          ...
          $scope.toggleWizardLoader.showLoader = false;
          $mdToast.show($mdToast.simple().content("offer " + $scope.offerName + " and locations Saved").position("top right").hideDelay(2700));
          $scope.clearOfferForm();
          if (goNext) {
            $state.go('core.home.campaign');
          }
      ...
      $scope.toggleWizardLoader.showLoader = false;
      $mdToast.show($mdToast.simple().content("error saving Offer location please try again").position("top right").hideDelay(2700));
   ...
   * @return  {object}  save offer success or fail
   */
  $scope.saveOfferTransactions = function (offerId, goNext) {
    /**
     * param passed to function
     *
     * @oID {string} offerId from API on $scope.saveOffer()
    ...
    var oID = offerId;
    var timeNow = moment().format("YYYY-MM-DD hh:mm:ss");
    var offerTransactions = $scope.conBuysLists;
    console.warn(JSON.stringify(offerTransactions, null, 4));

    offerTransactions.forEachCat(function (oFilter) {
      $timeout(function () {
        ...
        var offerTransaction = {
          ...
          Amount: isNaN($scope.transactionMainFilter.filter.offerAmount) ? $scope.transactionMainFilter.filter.offerAmount.amount === undefined ? $scope.getsAmount : $scope.transactionMainFilter.filter.offerAmount.amount : $scope.transactionMainFilter.filter.offerAmount,
          MerchantToken: merchantToken
        };
        Offer.createTransaction(offerTransaction).then(function (response) {
          var statusCode = response.FlikResponse.Status.StatusCode;
          var statusMessage = response.FlikResponse.Status.StatusMessage;

          if (statusCode == 0) {
            var _timeNow3 = moment().format("YYYY-MM-DD hh:mm:ss");
            var oTransactionID = response.FlikResponse.Data.MerchantOfferTransactionId[0];
            ...
            $rootScope.allData.offers.transactions.list.push({
              ...
              productCategory: Object.keys(oFilter.filter.productCat.selected).length > 0 ? oFilter.filter.productCat.selected.productCategory : "ALL",
              productCategoryDescription: Object.keys(oFilter.filter.productCat.selected).length > 0 ? oFilter.filter.productCat.selected.productCategoryDescription : "ALL",
              amount: isNaN($scope.transactionMainFilter.filter.offerAmount) ? $scope.transactionMainFilter.filter.offerAmount.amount === undefined ? 3 : $scope.transactionMainFilter.filter.offerAmount.amount : $scope.transactionMainFilter.filter.offerAmount
            });
            ...
            if (oFilter.filter.id === offerTransactions.length) {
              $scope.toggleWizardLoader.showLoader = false;
              $mdToast.show($mdToast.simple().content("offer " + $scope.offerName + " and redemtion filters saved").position("top right").hideDelay(2700));
              $scope.clearOfferForm();
              if (goNext) {
                $state.go('core.home.campaign');
              }
          ...
          $scope.toggleWizardLoader.showLoader = false;
          $mdToast.show($mdToast.simple().content("error saving Offer transaction please try again").position("top right").hideDelay(2700));
          console.warn("this is an error from the API " + JSON.stringify(error, null, 4));
        });
      }, 1200);
    });
  ...
  *
  **/
  $scope.saveOfferNextStep = function () {
   ...
   *
   */
  $scope.filterTime = function (isWhat) {
    if (isWhat === "category") {
      $timeout(function () {
  ...
  $scope.filterByProductCategory = function () {
    var productBrands = [];
    var selectedCategory = $scope.transactionMainFilter.filter.productCat.selected;
    var productBrand = $rootScope.allData.products.list;
  ...
  $scope.filterByProductBrand = function () {
    var productNames = [];
    var selectedCategoryName = $scope.transactionMainFilter.filter.productBrand.selected;
    var productName = $rootScope.allData.products.list;
...
//# sourceMappingURL=core.offer.create.js.map