...
app.controller("CoreFingerprintCreateCtrl", ["$scope", "$rootScope", "locker", "Fingerprint", "$http", "$filter", "$state", "$stateParams", "$mdToast", "$timeout", "Cause", "Store", "Product", function ($scope, $rootScope, locker, Fingerprint, $http, $filter, $state, $stateParams, $mdToast, $timeout, Cause, Store, Product) {
  ...
  if (!$stateParams.id) {
    locker.namespace("fingerprintCreate").bind($scope, "fingerprintName");
    $scope.fingerprintName = "";
    locker.namespace("fingerprintCreate").bind($scope, "rangeIncome.minIncome");
    locker.namespace("fingerprintCreate").bind($scope, "rangeIncome.maxIncome");
    $scope.rangeIncome = {
      minIncome: 1,
      maxIncome: 1000000
    };
    locker.namespace("fingerprintCreate").bind($scope, "rangeAge.minAge");
    locker.namespace("fingerprintCreate").bind($scope, "rangeAge.maxAge");
    $scope.rangeAge = {
      minAge: 16,
      maxAge: 60
    };
  ...
  var sFilter = $scope.stransactionFilter;
  var cFilter = $scope.ctransactionFilter;
  ...
  $scope.createConBuySocialList = function () {
    var filterId = $scope.conBuysSocialLists.length + 1;
  ...
  $scope.createConBuyCauseList = function () {
    var filterId = $scope.conBuysCauseLists.length + 1;
  ...
  $scope.trashConBuySocialList = function (index) {
    $scope.conBuysSocialLists.splice(index, 1);
  };

  $scope.trashConBuyCauseList = function (index) {
  ...
  $scope.refreshAddresses = function (address) {
    ...
    return $http.get('http://maps.googleapis.com/maps/api/geocode/json', { params: params }).then(function (response) {
      var originalAddresses = response.data.results;
  ...
  $scope.refreshProductBrand = function (productBrandSearch) {
  ...
  $scope.addressFormate = function (address) {
    var sepratedAdress = address[0].formatted_address.split(',');
    var city = undefined;
    var state = undefined;
  ...
  $scope.clearFingerprintForm = function () {
    $scope.fingerprintName = "";
    $scope.gender = {
      filter: "BOTH"
    };
   ...
   */
  $scope.saveFingerprintNextStep = function () {
    var goToOffer = true;
   ...
   */
  $scope.saveFingerprint = function (goNext) {
    $scope.toggleWizardLoader.showLoader = true;
    var fName = $scope.fingerprintName;

    if (!fName) {
      $scope.toggleWizardLoader.showLoader = false;
      $mdToast.show($mdToast.simple().content("Please enter a Fingerprint name").position("top right").hideDelay(2700));
      return;
    }
    ...
    Fingerprint.create(fingerprint).then(function (response) {
      ...
      if (statusCode == 0) {
        ...
        var fID = response.FlikResponse.Data.FingerPrint.FingerPrintId;
        var goToOffer = true;
      ...
      $scope.toggleWizardLoader.showLoader = false;
      $mdToast.show($mdToast.simple().content("error saving Fingerprint " + fName + " please try again").position("top right").hideDelay(2700));
     ...
     */
    $scope.saveFingerprintLocation = function (fID, goNext) {
      var timeNow = moment().format("YYYY-MM-DD hh:mm:ss");
      var fullLocation = $scope.fingerprintLocationAddresses.selected;
      var shouldSaveLocation = Object.keys(fullLocation).length > 0;
      ...
      if (!shouldSaveLocation) {
        ...
        Fingerprint.createLocation(fingerprintLocation).then(function (response) {
          var statusCode = response.FlikResponse.Status.StatusCode;
          var statusMessage = response.FlikResponse.Status.StatusMessage;
          if (statusCode == 0) {
            var _timeNow2 = moment().format("YYYY-MM-DD hh:mm:ss");
            var fLocationID = response.FlikResponse.Data.MerchantFingerprintLocation.FingerprintLocationId;
          ...
          $scope.toggleWizardLoader.showLoader = false;
          $mdToast.show($mdToast.simple().content("there was an error saving Fingerprint location please try again " + error).position("top right").hideDelay(2700));
     ...
     */
    $scope.saveFingerprintTransactions = function (fID, goNext) {
      var timeNow = moment().format("YYYY-MM-DD hh:mm:ss");
      var fingerprintTransactions = $scope.conBuysLists;
      var shouldSaveTransaction = Object.keys(fingerprintTransactions[0].filter.productCat.selected).length > 0 ? true : false;
      ...
      if (!shouldSaveTransaction) {
        ...
        fingerprintTransactions.forEachCat(function (fFilter) {
          $timeout(function () {
            ...
            Fingerprint.createTransaction(fingerprintTransaction).then(function (response) {
              var statusCode = response.FlikResponse.Status.StatusCode;
              var statusMessage = response.FlikResponse.Status.StatusMessage;
              if (statusCode == 0) {
                var _timeNow3 = moment().format("YYYY-MM-DD hh:mm:ss");
                var fTransactionID = response.FlikResponse.Data.MerchantFingerprintTransactionId[0];
    ...
    $scope.saveSocialHistoryTransactions = function (fID, goNext) {
      var timeNow = moment().format("YYYY-MM-DD hh:mm:ss");
      var fingerprintSocialTransactions = $scope.conBuysSocialLists;
      var shouldSaveSocialTransaction = Object.keys(fingerprintSocialTransactions[0].sfilter.productCat.selected).length > 0 ? true : false;;
      ...
      if (!shouldSaveSocialTransaction) {
        ...
        fingerprintSocialTransactions.forEachCat(function (sFilter) {
          $timeout(function () {
            ...
            var fingerprintSocialTransaction = {
              MerchantFingerprintId: fID,
            ...
            Fingerprint.saveSocialHistory(fingerprintSocialTransaction).then(function (response) {
              var statusCode = response.FlikResponse.Status.StatusCode;
              var statusMessage = response.FlikResponse.Status.StatusMessage;
              if (statusCode == 0) {
                var sTransactionID = response.FlikResponse.Data.MerchantFingerprintProductNew.MerchantFingerprintProductId;
                ...
                if (sFilter.sfilter.id === fingerprintSocialTransactions.length) {
                  locker.driver("session").namespace("fingerprintCreate").put("activeFingerprintSocialTransactionId", sTransactionID);
                  if (goNext) {
                    $scope.saveCauseHistoryTransactions(fID, goNext);
                  } else {
                    $scope.saveCauseHistoryTransactions(fID);
    ...
    $scope.saveCauseHistoryTransactions = function (fID, goNext) {
      var timeNow = moment().format("YYYY-MM-DD hh:mm:ss");
      var fingerprintCauseTransactions = $scope.conBuysCauseLists;
      var shouldSaveCauseTransaction = Object.keys(fingerprintCauseTransactions[0].cfilter.causeName.selected).length > 0 ? true : false;

      if (!shouldSaveCauseTransaction) {
        console.log("no cause transactions to save");
        $scope.toggleWizardLoader.showLoader = false;
        $scope.clearFingerprintForm();
        $mdToast.show($mdToast.simple().content("Fingerprint " + $scope.fingerprintName + " with all targeting Saved").position("top right").hideDelay(2700));
        if (goNext) {
          $state.go('core.home.offer');
        }
      } else {
        fingerprintCauseTransactions.forEachCat(function (cFilter) {
          $timeout(function () {
            var fingerprintCauseTransaction = {
              MerchantFingerprintId: fID,
            ...
            Fingerprint.saveCauseHistory(fingerprintCauseTransaction).then(function (response) {
              var statusCode = response.FlikResponse.Status.StatusCode;
              var statusMessage = response.FlikResponse.Status.StatusMessage;
              if (statusCode == 0) {
                var cTransactionID = response.FlikResponse.Data.MerchantFingerprintCauseNew.MerchantFingerprintCauseId;
                ...
                if (cFilter.cfilter.id === fingerprintCauseTransactions.length) {
                  $scope.toggleWizardLoader.showLoader = false;
                  $mdToast.show($mdToast.simple().content("Fingerprint saved").position("top right").hideDelay(2700));
                  locker.driver("session").namespace("fingerprintCreate").put("activeFingerprintCauseTransactionId", cTransactionID);
                  $scope.clearFingerprintForm();
                  if (goNext) {
                    $state.go('core.home.offer');
                  }
    ...
    $scope.saveStore = function (fID, goNext) {
      var timeNow = moment().format("YYYY-MM-DD hh:mm:ss");
      var storeLocation = $scope.fingerprintStoreAddresses.selected;
      var beacon = $scope.fingerprintBeaconAddresses.selected;
      var shouldSaveStoreLocation = Object.keys(storeLocation).length > 0;
      var shouldSaveBeacon = Object.keys(beacon).length > 0;
      ...
      if (!shouldSaveStoreLocation) {
        ...
        Fingerprint.storeSave(storeInfo).then(function (response) {
          var statusCode = response.FlikResponse.Status.StatusCode;
          var statusMessage = response.FlikResponse.Status.StatusMessage;
          if (statusCode == 0) {
            var storeID = response.FlikResponse.Data.MerchantFingerprintStore.MerchantFingerprintStoreId;
            locker.driver("session").namespace("fingerprintCreate").put("activeFingerprintStoreTransactionId", storeID);
    ...
    $scope.beaconSave = function (fID, goNext) {
      var timeNow = moment().format("YYYY-MM-DD hh:mm:ss");
      var beaconInfo = {
        MerchantFingerprintId: fID,
        StoreId: $scope.fingerprintStoreAddresses.selected.storeId,
      ...
      Fingerprint.beaconSave(beaconInfo).then(function (response) {
        var statusCode = response.FlikResponse.Status.StatusCode;
        var statusMessage = response.FlikResponse.Status.StatusMessage;
        if (statusCode == 0) {
          var beaconID = response.FlikResponse.Data.Beacons.MerchantFingerprintBeaconsId;
          locker.driver("session").namespace("fingerprintCreate").put("activeFingerprintBeaconTransactionId", beaconID);
          ...
          var storeTransactionId = locker.driver("session").namespace("fingerprintCreate").get("activeFingerprintStoreTransactionId", beaconID);
          $rootScope.allData.fingerprints.storeTransactions.list.push({
            merchantFingerprintId: fID,
...
//# sourceMappingURL=core.fingerprint.create.js.map