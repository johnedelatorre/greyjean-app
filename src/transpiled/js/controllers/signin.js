...
// signin controller
app.controller('SigninFormController', ['$scope', '$rootScope', 'Auth', 'Fingerprint', 'Offer', 'Campaign', 'Product', '$filter', '$state', 'locker', 'Cause', 'Store', 'Beacon', 'Analytics', '$cookies', function ($scope, $rootScope, Auth, Fingerprint, Offer, Campaign, Product, $filter, $state, locker, Cause, Store, Beacon, Analytics, $cookies) {
  ...
  $scope.login = function () {
    ...
    Auth.login(merchantCreds).then(function (response) {
      ...
      if (statusCode == 0) {
        (function () {
          var merchantToken = response.FlikResponse.Data.MerchantToken.Token;
          ...
          Auth.getMe(merchantToken).then(function (response) {
            if (statusCode == 0) {
              ...
              locker.driver('local').namespace('core').put('me', merchantInfo);
              ...
              //          });
                ...
                locker.driver('local').namespace('core').forget(['merchantToken', 'me']);
