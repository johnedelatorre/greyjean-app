 ...
 */
angular.module("app").run(["$rootScope", "$state", "locker", "Auth", "Restangular", "$stateParams", "editableOptions", function ($rootScope, $state, locker, Auth, Restangular, $stateParams, editableOptions) {
   ...
   */
  $rootScope.trashMerchantData = function () {
    locker.driver("local").namespace("core").forget(["merchantToken", "me", "allData"]);
...
.config(['$stateProvider', '$urlRouterProvider', function ($stateProvider, $urlRouterProvider) {
  ...
  $stateProvider.state('core', {
    ...
    controller: function controller($scope, Analytics, locker, $filter, $mdToast) {
      var tempcampall = [];
        ...
        if (statusCode == 0) {
          ...
          $scope.updatecampfilterresponse = function (tempcampall) {
            ...
            $scope.camapignGraph = function (dayData) {
              var thirtyDayParticipantInformation = parseInt(dayData.thirtyDaysCampaignTotalParticipants);
              var sixtyDayParticipantInformation = parseInt(dayData.sixtyDaysCampaignTotalParticipants);
              var nintyDayParticipantInformation = parseInt(dayData.nintyDaysCampaignTotalParticipants);
            ...
            $scope.redamptionGraph = function (dayData) {
              var thirtyDayRedemptionInformation = parseFloat(dayData.thirtyDaysCampaignTotalRedemptionRate);
              var sixtyDayRedemptionInformation = parseFloat(dayData.sixtyDaysCampaignTotalRedemptionRate);
              var nintyDayRedemptionInformation = parseFloat(dayData.nintyDaysCampaignTotalRedemptionRate);
            ...
            $scope.salesGraph = function (dayData) {
              var thirtyDaySalesInformation = parseInt(dayData.thirtyDaysCampaignTotalSalesAmountforAllRedemptions);
              var sixtyDaySalesInformation = parseInt(dayData.sixtyDaysCampaignTotalSalesAmountforAllRedemptions);
              var nintyDaySalesInformation = parseInt(dayData.nintyDaysCampaignTotalSalesAmountforAllRedemptions);
            ...
            $scope.applycampfilter = function (response) {
              ...
              var overallResponse = response.FlikResponse.Data.AnalyticsFiltering.TotalAnalyticsResult;
            ...
            $scope.campaignFilterApply = function () {
              if ($scope.isCampaignChecked.data) {
                var idInString = $scope.ids.toString();
                var _merchantRefId = {
                  MerchantToken: locker.driver("local").namespace("core").get("merchantToken"),
                ...
                if ($scope.analyticscampaigntype.value == 'Mobile') {
                  Analytics.getFilterData(_merchantRefId).then(function (response) {
                    $scope.applycampfilter(response);
                  });
                } else if ($scope.analyticscampaigntype.value == 'Social') {
                  Analytics.getSocialFilterData(_merchantRefId).then(function (response) {
                ...
                $scope.filterdLists = $scope.campaignInfo;
                var overallResponse = $scope.campaignAll;
                $scope.salesSummary = overallResponse[0];
                $scope.camapignGraph($scope.salesSummary);
                $scope.redamptionGraph($scope.salesSummary);
                $scope.salesGraph($scope.salesSummary);
            ...
            $scope.offerFilterApply = function () {
              if ($scope.isOfferChecked.data) {
                ...
                if ($scope.analyticscampaigntype.value == 'Mobile') {
                  Analytics.getFilterData(_merchantRefId3).then(function (response) {
                    $scope.applyofferfilter(response);
                  });
                } else if ($scope.analyticscampaigntype.value == 'Social') {
                  Analytics.getSocialFilterData(_merchantRefId3).then(function (response) {
                    $scope.applyofferfilter(response);
                  });
                }
              } else {
            ...
            $scope.fingerprintFilterApply = function () {
              if ($scope.isFingerprintChecked.data) {
                ...
                if ($scope.analyticscampaigntype.value == 'Mobile') {
                  Analytics.getFilterData(_merchantRefId5).then(function (response) {
                    $scope.applyfingerfilter(response);
                  });
                } else if ($scope.analyticscampaigntype.value == 'Social') {
                  Analytics.getSocialFilterData(_merchantRefId5).then(function (response) {
                    $scope.applyfingerfilter(response);
                  });
                }
              } else {
            ...
            $scope.endDate = function (selectedEndDate, isSelect) {
              ...
              if ($scope.analyticscampaigntype.value == 'Mobile') {
                Analytics.getFilterData(merchantRefId).then(function (response) {
                  $scope.applyenddate(response);
                });
              } else if ($scope.analyticscampaigntype.value == 'Social') {
                Analytics.getSocialFilterData(merchantRefId).then(function (response) {
                  $scope.applyenddate(response);
                });
              }
            };
          ...
          if ($scope.analyticscampaigntype.value == 'Mobile') {
            Analytics.getFilterData(merchantRef).then(function (resp) {
              ...
              $scope.toggleWizardLoader.showLoader = false;
              $mdToast.show($mdToast.simple().content("Error in loading filter data").position("top right").hideDelay(2700));
            });
          } else if ($scope.analyticscampaigntype.value == 'Social') {
            Analytics.getSocialFilterData(merchantRef).then(function (resp) {
              $scope.campaignAll = resp.FlikResponse ? resp.FlikResponse.Data.AnalyticsFiltering.TotalAnalyticsResult : [];
              tempcampall = $scope.campaignAll;
              $scope.updatecampfilterresponse(tempcampall);
            }, function (error) {
      ...
      $scope.updateanalytics = function () {
        if ($scope.analyticscampaigntype.value == 'Mobile') {
          Analytics.getCampaignAll(merchantRef).then(function (response) {
            ...
            $scope.toggleWizardLoader.showLoader = false;
            $mdToast.show($mdToast.simple().content("Error in getting campaign Data").position("top right").hideDelay(2700));
          });
        } else if ($scope.analyticscampaigntype.value == 'Social') {
          Analytics.getSocialCampaignAll(merchantRef).then(function (response) {
            var statusCode = response.FlikResponse.Status.StatusCode;
            var statusMessage = response.FlikResponse.Status.StatusMessage;
            $scope.updatecampresponse(response, statusCode, statusMessage);
          }, function (error) {
    ...
    controller: function controller($scope, $rootScope, Analytics, Product, Beacon, Cause, Store) {
      ...
      $scope.allConsumer = function () {
        var numberOfConsumer = {
          NumberOfConsumer: $scope.consumer.range,
          MerchantToken: locker.driver("local").namespace("core").get("merchantToken")
        };

        Analytics.getConsumerAll(numberOfConsumer).then(function (response) {
          var statusCode = response.FlikResponse.Status.StatusCode;
          var statusMessage = response.FlikResponse.Status.StatusMessage;
          if (statusCode == 0) {
            $scope.ConsumerRows = {
      ...
      $scope.allConsumerRecommendation = function () {
        ...
        Analytics.getConsumerrecommendationsAll(numberOfConsumer).then(function (response) {
          var statusCode = response.FlikResponse.Status.StatusCode;
          var statusMessage = response.FlikResponse.Status.StatusMessage;
          if (statusCode == 0) {
      ...
      $scope.itemSimilarity = function () {
        ...
        Analytics.getItemSimilarityAll(numberOfItems).then(function (response) {
          var statusCode = response.FlikResponse.Status.StatusCode;
          var statusMessage = response.FlikResponse.Status.StatusMessage;
          if (statusCode == 0) {
      ...
      $scope.allProductCorrelations = function () {
        ...
        Analytics.getProductCorrelationsAll(MerchantToken).then(function (response) {
          var statusCode = response.FlikResponse.Status.StatusCode;
          var statusMessage = response.FlikResponse.Status.StatusMessage;
          if (statusCode == 0) {
      ...
      $scope.allCampaigns = function () {
        ...
        Analytics.getCampaignAll(MerchantToken).then(function (response) {
          var statusCode = response.FlikResponse.Status.StatusCode;
          var statusMessage = response.FlikResponse.Status.StatusMessage;
          if (statusCode == 0) {
            $scope.campaignRows = {
      ...
      $scope.allBeacons = function () {
        ...
        Beacon.beaconGetAll(MerchantToken).then(function (response) {
          var statusCode = response.FlikResponse.Status.StatusCode;
          var statusMessage = response.FlikResponse.Status.StatusMessage;
          if (statusCode == 0) {
            $scope.beaconRows = {
      ...
      $scope.allCauses = function () {
        ...
        Cause.causeGetAll(MerchantToken).then(function (response) {
          var statusCode = response.FlikResponse.Status.StatusCode;
          var statusMessage = response.FlikResponse.Status.StatusMessage;
          if (statusCode == 0) {
            $scope.causeRows = {
      ...
      $scope.allStores = function () {
        ...
        Store.storeGetAll(MerchantToken).then(function (response) {
          var statusCode = response.FlikResponse.Status.StatusCode;
          var statusMessage = response.FlikResponse.Status.StatusMessage;
          if (statusCode == 0) {
            $scope.storesRows = {
      ...
      $scope.allProducts = function () {
        ...
        Product.getAll(MerchantToken).then(function (response) {
          var statusCode = response.FlikResponse.Status.StatusCode;
          var statusMessage = response.FlikResponse.Status.StatusMessage;
          if (statusCode == 0) {
            $scope.productRows = {
      ...
      $scope.consumerTransaction = function () {
        ...
        Analytics.getConsumerTransactionAll(transactionData).then(function (response) {
          var statusCode = response.FlikResponse.Status.StatusCode;
          var statusMessage = response.FlikResponse.Status.StatusMessage;
          if (statusCode == 0) {
      ...
      $scope.productWithTransaction = function () {
        ...
        Analytics.getProductWithTransactionAll(MerchantToken).then(function (response) {
          var statusCode = response.FlikResponse.Status.StatusCode;
          var statusMessage = response.FlikResponse.Status.StatusMessage;
          if (statusCode == 0) {
            $scope.productWithTransactionRows = {
    ...
    resolve: {
      ...
      //  })
      //},
      fetchStoreData: function fetchStoreData(Fingerprint, $stateParams, locker) {
        var merchantRef = {
          MerchantToken: locker.driver("local").namespace("core").get("merchantToken")
        };
        return Fingerprint.storeGetAll(merchantRef).then(function (resp) {
          return resp;
        });
      },
      deps: ['$ocLazyLoad', function ($ocLazyLoad) {
    ...
    views: {
      'fingerprintWidget': {
        ...
        controller: function controller($scope, $rootScope, $stateParams, $filter, fetchStoreData, Fingerprint) {
          ...
          var socialTransactions = $rootScope.allData.fingerprints.socialTransactions.list;
          var fingerprintId = $stateParams.id;
          ...
          Fingerprint.FingerprintAllData(merchantToken).then(function (resp) {
            ...
            var fingerprintList = $scope.allFingerprints;
            var locationList = $scope.allFingerprintLocations;
            var transactionList = $scope.allFingerprintTransactions;
            var causeList = $rootScope.allData.fingerprints.causeTransactions.list;
            var productList = $rootScope.allData.products.list;
            var productWithTransactionList = $rootScope.allData.productsWithTransaction.list;
            var storeTransactionList = $rootScope.allData.fingerprints.storeTransactions.list;
            var storeList = $rootScope.allData.stores.list;
            var beaconTransactionList = $rootScope.allData.fingerprints.beaconTransactions.list;
            var beaconList = $rootScope.allData.beacons.list;
            ...
            var shouldShowLocation = Object.keys($scope.fingerprintData.locations).length > 0 ? true : false;
            var shouldShowTransaction = Object.keys($scope.fingerprintData.transactions).length > 0 ? true : false;
            ...
            var shouldShowSocialTransaction = Object.keys($scope.socialFingerprintData.transactions).length > 0 ? true : false;
            ...
            var shouldShowCauseTransaction = Object.keys($scope.causeFingerprintData.transactions).length > 0 ? true : false;
            ...
            var shouldShowBeacon = Object.keys($scope.beaconFingerprintData.transactions).length > 0 ? true : false;
            var shouldShowStore = Object.keys($scope.storeFingerprintData.transactions).length > 0 ? true : false;
            if (shouldShowBeacon && !shouldShowStore) {
              ...
              var beaconId = $scope.beaconFingerprintData.transactions[0].beaconId;
              var storeId = $scope.beaconFingerprintData.transactions[0].storeId;
              $scope.fingerprintStoreAddresses = {
                selected: $filter('where')(storeList, { storeId: storeId })[0]
              };
              locker.namespace("fingerprintCreate").bind($scope, "fingerprintBeaconAddresses.selected");
              ...
              var storeId = $scope.storeFingerprintData.transactions[0].storeId;
              locker.namespace("fingerprintCreate").bind($scope, "fingerprintStoreAddresses.selected");
              $scope.fingerprintStoreAddresses = {
                selected: $filter('where')(storeList, { storeId: storeId })[0]
              };
            } else if (!shouldShowBeacon && !shouldShowStore) {
            ...
            //                  locker.driver("session").namespace("fingerprintCreate").forget("activeFingerprintStoreTransactionId");
            //                }
            var thisFingerprint = $scope.fingerprintData.fingerprint[0];
            if ($scope.fingerprintData.locations.length > 0) {
              var formattedAddresses = [$scope.fingerprintData.locations[0].streetAddress, $scope.fingerprintData.locations[0].city, $scope.fingerprintData.locations[0].state, $scope.fingerprintData.locations[0].country];
              var formattedAddress = formattedAddresses.join(',');
              var locationData = {
              ...
              locker.namespace("fingerprintCreate").bind($scope, "fingerprintLocationAddresses.selected");
              $scope.fingerprintLocationAddresses.selected = locationData;
            }

            locker.namespace("fingerprintCreate").bind($scope, "radius.selected");
            $scope.radius = [{ radius: "1" }, { radius: "5" }, { radius: "10" }, { radius: "15" }, { radius: "20" }];

            var radius = (fingerprintList, { fingerprintId: fingerprintId })[0] = "undefined" ? " " : $filter('where')(fingerprintList, { fingerprintId: fingerprintId })[0].radius;
            $scope.radius.selected = { radius: radius };

            locker.namespace("fingerprintCreate").bind($scope, "gender.filter");
            $scope.gender = {
            ...
            locker.namespace("fingerprintCreate").bind($scope, "conBuysLists");
            //$scope.conBuysLists = $scope.fingerprintData.transactions.length > 0 ? [] : [filter];
            $scope.conBuysLists = [];
            $scope.fingerprintData.transactions.forEachCat(function (transaction) {
            ...
            ///social
            locker.namespace("fingerprintCreate").bind($scope, "conBuysSocialLists");
            $scope.conBuysSocialLists = [];
            $scope.socialFingerprintData.transactions.forEachCat(function (transaction) {
            ...
            ///cause
            locker.namespace("fingerprintCreate").bind($scope, "conBuysCauseLists");
            $scope.conBuysCauseLists = [];

            $scope.causeFingerprintData.transactions.forEachCat(function (transaction) {
    ...
    url: '/:id/update',
    resolve: {
      fetchOfferData: function fetchOfferData(Offer, locker) {
        var merchantRef = {
          MerchantToken: locker.driver("local").namespace("core").get("merchantToken")
        };
        return Offer.getAllData(merchantRef).then(function (resp) {
          return resp;
        });
      },
      deps: ['$ocLazyLoad', function ($ocLazyLoad) {
    ...
    views: {
      'offerWidget': {
        ...
        controller: function controller($scope, $rootScope, $stateParams, $filter, fetchOfferData) {
          var resp = fetchOfferData.FlikResponse.Data;
          ...
          $scope.creditAmounts = [{ amount: "1" }, { amount: "2" }, { amount: "3" }, { amount: "4" }, { amount: "5" }];
          ...
          locker.namespace("offerCreate").bind($scope, "offerLocationAddresses.selected");
          $scope.offerLocationAddresses.selected = $scope.offerData.locations[0];

          locker.namespace("offerCreate").bind($scope, "conBuysLists");
          $scope.conBuysLists = [];
          ...
          if ($scope.offerData.transactions.length > 0) {
            ...
            $scope.offerData.transactions.forEachCat(function (transaction) {
              $scope.conBuysLists.push({
                filter: {
    ...
    url: '/:id/update',
    resolve: {
      fetchCampaign: function fetchCampaign(Campaign, $stateParams, locker) {
        var campaignRef = {
          CampaignId: $stateParams.id,
          MerchantToken: locker.driver("local").namespace("core").get("merchantToken")
        };
        //console.log("this is the ref " + JSON.stringify(campaignRef, null, 4));
        return Campaign.getOne(campaignRef).then(function (resp) {
          return resp;
        });
      },
      deps: ['$ocLazyLoad', function ($ocLazyLoad) {
    ...
    views: {
      'campaignWidget': {
        ...
        controller: function controller($scope, fetchCampaign, $rootScope, $filter, Campaign) {
          var resp = fetchCampaign.FlikResponse.Data;
          var campaign = resp.MerchantCampaign.CampaignGet;
          var activeFingerprintId = campaign.fingerprintId;
          var activeOfferId = campaign.offerId;
  ...
  .state('access', {
    ...
    onEnter: function onEnter($rootScope, locker) {
      /**
