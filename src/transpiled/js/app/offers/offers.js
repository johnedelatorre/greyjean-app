...
app.controller("OffersCtrl", ["$scope", "$rootScope", "$http", "$mdToast", "Offer", "$filter", function ($scope, $rootScope, $http, $mdToast, Offer, $filter) {
  ...
  var merchantOffers = $rootScope.allData.offers;
  $scope.allOffers = {};
  ...
  $scope.trashOffer = function (index, oId) {
    ...
    Offer.trash(offerId).then(function (response) {
      ...
      if (statusCode == 0) {
        ...
        $mdToast.show($mdToast.simple().content("offer deleted successfully").position("top right").hideDelay(2700));
      ...
      $mdToast.show($mdToast.simple().content("there was an error deleting offer please try again").position("top right").hideDelay(2700));
